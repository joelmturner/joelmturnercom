...
async function getPostData(slug: string): Promise<TIL[]> {
  const posts = getAllPostsByCategory(slugify(slug), 'til') as TIL[];
  return posts;
}

export async function generateStaticParams() {
  return getAllCategories('til');
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: {
      canonical: `https://joelmturner.com/til/category/${params.slug}`,
    },
    category: params.slug,
    openGraph: {
      title,
      description: `Posts about ${params.slug}`,
      url: `https://joelmturner.com/til/category/${params.slug}`,
