...
tags:
  ...
  - TypeScript
  - Cloudinary
lastmod: 2022-06-13T22:29:24.946Z
excerpt: Setting up a GitHub Action to upload images to Cloudinary from Instagram. This
  solves some of the issues that I was having with getting Instagram images into
  Gatsby.
cover: https://res.cloudinary.com/joelmturner/image/upload/v1655159350/Posts/instagram_to_cloudinary.jpg
---
...
```typescript filename=src/index.ts {1-10,13}
...
```typescript filename=src/index.ts {2,8-10}
async function fetchInstagramPosts() {
  try {
    console.log("🚀 fetching Instagram posts");
    const response = await axios.get(
...
```typescript filename=src/index.ts {7-10,13}
...
```typescript filename=src/index.ts {3-51}
function convertInstagramPostToCloudinaryEntity(posts: Post[]): UploadPost[] {
  console.log("🚀 converting posts to Cloudinary");
...
```typescript filename=src/index.ts {12}
import { UploadPost } from "./types";
...
```typescript filename=src/index.ts {3-31}
async function sendToCloudinary(postGroup: UploadPost[]) {
  console.log("🚀 uploading to Cloudinary");
